...
* **Composición:** Hice que la clase `Temporada` se cree directamente por dentro de `SerieDeTV` y también en mi nueva clase.
* **Asociación:** Conecté la clase `Investigador` con el `Documental`.
* **Clases propias (Herencia):** Agregué dos clases nuevas llamadas `Tiktok` y `Facebook` que heredan de la clase principal `ContenidoAudiovisual`.
...
* **Reutilización:** Aproveché la clase `Temporada` que ya habíamos creado y la reutilicé dentro de mi clase inventada para organizar los arcos o temporadas sin tener que programar todo desde cero.
